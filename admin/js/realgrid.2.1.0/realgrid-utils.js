...
function defaultGridListSet(gridView){
	    //컬럼 너비 자동조절
		gridView.displayOptions.fitStyle = "even";
		gridView.displayOptions.rowFocusType='row';
		gridView.displayOptions.rowHoverType='row';
		gridView.displayOptions.selectionStyle ='none';
		gridView.displayOptions.selectionMode='single';
    ...
    //페이징 버튼 세팅 방법
	//그리드뷰 페이징 세팅 setRows 아래에 추가할것 param (그리드뷰 객체, 페이지 당 나타낼 행의 개수)
	//grid 블락 아래에 pagination.jsp 파일 include 시킬것  
	
	//이하 페이징 세팅 함수들 
	function gridPagingSetting(gridView,countPerPage){
		
		gridView.setPaging(true,countPerPage);
		
		//버튼 세팅
		$('ul > #pageBtn').click(function(){
			moveGridViewPaging(gridView,$(this).children().text());	
		});
		 
		$('#firstPageBtn').click(function(){
			moveGridViewPaging(gridView,1);	
		});
		$('#prevPageBtn').click(function(){
			moveGridViewPaging(gridView, parseInt(gridView.getPage()/10)*10);	
		});
		
		$('#nextPageBtn').click(function(){
			moveGridViewPaging(gridView,(parseInt(gridView.getPage()/10)+1)*10+1);	
		});
		$('#lastPageBtn').click(function(){
			moveGridViewPaging(gridView,gridView.getPageCount());	
		}); 
	...
	function moveGridViewPaging(gridView,pageNum){
		
		//if(isNaN(pageNum)) pageNum=1;
		
		//이동하고자 하는 페이지 
		gridView.setPage(pageNum-1);
		
		//총 페이지
		var total = gridView.getPageCount();
		
		//현 페이지의 따른 페이지 버튼들 세팅 
		for(var i=0;i<10;i++){
				var aTag = $('#pagingBtns').children("li:eq(" + i + ")").children();
				pageBtnSet(aTag,parseInt((pageNum-1)/10)*10+i+1,pageNum,total);
		}
		
		//이전, 다음, 처음, 마지막 버튼 활성화,비활성화
		$('#firstPageBtn').toggle(pageNum>10);
		$('#prevPageBtn').toggle(pageNum>10);
		$('#nextPageBtn').toggle(total>=parseInt((pageNum-1)/10+1)*10);
		$('#lastPageBtn').toggle(total>=parseInt((pageNum-1)/10+1)*10);
		/*
		if(pageNum == 1){
			$('#firstPageBtn').hide();
			$('#prevPageBtn').hide();
	...
	function pageBtnSet(obj,idx,now,total){
		obj.text(idx);
		//버튼을 파랗게 칠하는 부분 
		if(now == idx) {
		...
		//페이징이 안되어야할 페이지일 경우 비 활성화 (토탈 페이지를 넘어가거나, 0이하의 페이지)
		if(idx>total || idx<1){
			obj.hide();
		} else{
			obj.show();
